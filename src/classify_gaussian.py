"""
classify_gaussian.py — Gaussian MLE Naive Bayes classifier.

Loads per-class mu/sigma from models/params_gaussian.json, runs log-space
Gaussian NB, optimises threshold via Youden's J on the training set, and
evaluates on the held-out test set.

Usage:
    python src/classify_gaussian.py             # all 5 features
    python src/classify_gaussian.py f5          # single feature
    python src/classify_gaussian.py f1 f3 f5    # feature subset
"""
...
def classify_one(raw: dict, params: dict, features: tuple) -> float:
    ...
    vals = {
        feat: math.log1p(raw[feat]) if cls0[feat]['log_transform'] else raw[feat]
        for feat in raw
    }
...
def plot_confusion_matrix(cm, out_path):
    ...
    ax.set(xticks=[0, 1], yticks=[0, 1],
           ...
           title='Confusion Matrix (Gaussian MLE-NB)')
    for i in range(2):
        for j in range(2):
...
def plot_roc(y_true, y_scores, auc, out_path):
    ...
    ax.set(xlabel='False Positive Rate', ylabel='True Positive Rate',
           title='ROC Curve (Gaussian MLE-NB)')
    ax.legend()
    plt.tight_layout()
    plt.savefig(out_path, dpi=150)
    plt.close()
...
def plot_score_distribution(y_true, y_scores, out_path, threshold):
    ...
    ax.set(xlabel='P(Deepfake)', ylabel='Count', title='Score Distribution (Gaussian MLE-NB)')
    ax.legend()
    plt.tight_layout()
    plt.savefig(out_path, dpi=150)
    plt.close()
...
def evaluate(params_path=PARAMS_JSON, features_path=FEATURES_CSV, results_dir=RESULTS_DIR,
    ...
    print(f"\n{'='*40}")
    print(f"  [Gaussian MLE-NB -- features: {features_subset}]")
    ...
    tag = 'gaussian_' + '_'.join(features_subset)
    ...
    metrics = {
        "experiment": f"Gaussian-MLE-NB features={list(features_subset)}",
        "threshold": threshold,
